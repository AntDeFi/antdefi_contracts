...
contract AntToken is BEP20 {
    ...
    // Min amount to liquify. (default 9660 ANTs)
    uint256 public minAmountToLiquify = 9660 ether;
    // The swap router, modifiable. Will be changed to AntSwap's router when our own AMM release
    IUniswapV2Router02 public antSwapRouter;
    // The trading pair
    address public antSwapPair;
    // In swap and liquify
    ...
    event AntSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    /**
     * @notice Constructs the AntToken contract.
     */
    constructor() public BEP20("Ant Token", "ANT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && _inSwapAndLiquify == false
            && address(antSwapRouter) != address(0)
            && antSwapPair != address(0)
            && sender != antSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = antSwapRouter.WETH();

        _approve(address(this), address(antSwapRouter), tokenAmount);

        // make the swap
        antSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        antSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from antSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "ANT::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "ANT::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateAntSwapRouter(address _router) public onlyOperator {
        antSwapRouter = IUniswapV2Router02(_router);
        antSwapPair = IUniswapV2Factory(antSwapRouter.factory()).getPair(address(this), antSwapRouter.WETH());
        require(antSwapPair != address(0), "ANT::updateAntSwapRouter: Invalid pair address.");
        emit AntSwapRouterUpdated(msg.sender, address(antSwapRouter), antSwapPair);
    }
