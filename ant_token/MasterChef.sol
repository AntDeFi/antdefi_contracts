...
import "./libs/IAntReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

import "./AntToken.sol";

// MasterChef is the master of Ant. He can make Ant and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ANT is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Ant referral contract address.
    IAntReferral public antReferral;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 antReward = multiplier.mul(antPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ant.mint(devAddress, antReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(antReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            antReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingAnt(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(ant)) {
                uint256 transferTax = _amount.mul(ant.transferTaxRate()).div(10000);
    ...
    // Update the ant referral contract address by the owner
    function setAntReferral(IAntReferral _antReferral) public onlyOwner {
        antReferral = _antReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(antReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = antReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                ant.mint(referrer, commissionAmount);
                antReferral.recordReferralCommission(referrer, commissionAmount);
